...
namespace MonopolyFinal
{
    public class Board
    {
        ...
        public string PlayOneTurn()
        {
            ...
            {
                Recalculate();
                List<TradeAction> tradeoffer = new List<TradeAction>();
        ...
        public List<Array> GetBoardState()
        {
            List<Array> boardState = new List<Array>();

            for(int i = 0; i < Gameboard.Length; i++)
            {
                if (Gameboard[i] is SimpleBuyableTiles)
                {
                    int[] tileInBoardState = new int[2];
                    SimpleBuyableTiles castedTile = (SimpleBuyableTiles)Gameboard[i];

                    Player owner = castedTile.Owner;
                    if (owner != null) tileInBoardState[0] = int.Parse(owner.Name.Last().ToString());
                    else tileInBoardState[0] = -1;

                    if (castedTile is ComplexBuyableTiles) 
                    {
                        if (owner != null)
                            tileInBoardState[1] = castedTile.NoHouses + 1;
                        else tileInBoardState[1] = 0; 
                    }
                    else tileInBoardState[1] = 1;
                    boardState.Add(tileInBoardState);
                }
            }
            List<int> playerCash = new List<int>();
            foreach (Player player in Players)
            {
                playerCash.Add(player.Cash);
            }
            boardState.Add(playerCash.ToArray());
            return boardState;
        }

        public string GetBoardDataForPlayerOne()
        {
            string output = "";
            foreach(Tile tile in Gameboard)
            {
                if (tile is SimpleBuyableTiles)
                {
                    SimpleBuyableTiles castedTile = (SimpleBuyableTiles)tile;
                    if(castedTile.Owner == Players[0])
                    {
                        if (castedTile is ComplexBuyableTiles) output += castedTile.NoHouses + ",";
                        else output += 1 + ",";
                    }
                    else
                    {
